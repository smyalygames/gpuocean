...
echo "Submitting Dynamic dt MPI strong scaling test, with benchmark ID: ${benchmark_id}"

job=/project/project_465002898/anthony/gpuocean/jobs/benchmarks/strong_scaling/mpi.slurm
...
for processes in {0..2}; do
  ...
  sbatch \
    ...
    --time=01:30:00 \
    --output="out/GPUOcean-mpi-dynamic-dt-strong-%j-${total_tasks}.out" \
    $job "$benchmark_id"
done
