...
from gpuocean.utils.gpu import KernelContext, GPUStream, Array3D


class Array3DTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.nx = 9
        self.ny = 8
        self.dataShape = (self.ny, self.nx, self.nc)
        ...
        for i in range(self.dataShape[0]):
            for j in range(self.dataShape[1]):
                for k in range(self.dataShape[2]):
                    self.buf1[i, j, k] = j * 100 + i * 10 + k
                    self.dbuf1[i, j, k] = self.buf1[i, j, k]
                    self.buf3[i, j, k] = i * 1000  + j * 100 - k
                    self.dbuf3[i, j, k] = self.buf3[i, j, k]
        ...
        self.gpu_array = Array3D(self.gpu_stream,
                                 self.nx, self.ny, self.nc,
                                 self.buf1)

        self.double_gpu_array: Array3D | None = None
    ...
    def init_double(self):
        self.double_gpu_array = Array3D(self.gpu_stream,
                                        self.nx, self.ny, self.nc,
                                        self.dbuf1,
    ...
    def test_init(self):
        ...
        self.assertEqual(self.gpu_array.nc, self.nc)
    ...
    def test_copy_buffer(self):
        clarray2 = Array3D(self.gpu_stream,
                           self.nx, self.ny, self.nc,
                           self.buf3)
    ...
    def test_double_init(self):
        ...
        self.assertEqual(self.double_gpu_array.nc, self.nc)
    ...
    def test_double_copy_buffer(self):
        ...
        double_gpu_array2 = Array3D(self.gpu_stream,
                                    self.nx, self.ny, self.nc,
                                    self.dbuf3,
                                    double_precision=True)
    ...
    def test_cross_precision_copy_buffer(self):
        ...
        single_gpu_array2 = Array3D(self.gpu_stream,
                                    self.nx, self.ny, self.nc,
                                    self.buf3)
