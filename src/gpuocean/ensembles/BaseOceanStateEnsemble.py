...
class BaseOceanStateEnsemble(object):
    ...
    def __init__(self):
        ...
        # Not an abstract
        self.observation_type: dautils.ObservationType | None = None
            ...
            for p in range(self.getNumParticles()):
                ...
                if gpu:
                    ...
                    for d in range(self.getNumDrifters()):
                        id_x = int(np.floor(observations[d, 0] / self.dx))
                        id_y = int(np.floor(observations[d, 1] / self.dy))
    ...
    def getGaussianWeight(self, innovations=None, normalize=True, R_scale=1.0):
        ...
        if normalize:
            ...
            if len(innovations.shape) == 1:
                ...
                return (1.0 / ((2 * np.pi) ** numDrifters * np.linalg.det(R) ** (numDrifters / 2.0))) * np.exp(
                    log_weights)
    ...
    def downloadParticleOceanState(self, particleNo):
        ...
        """
        assert (self.getNumParticles() > particleNo > -1), "particle out of range"
