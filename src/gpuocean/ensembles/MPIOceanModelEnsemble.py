...
class MPIOceanModelEnsemble:
    ...
    def __init__(self, comm: MPI.Intracomm,
                 ...
                 sim_args: dict = None, data_args: dict = None,
                 ensemble_args: dict = None, metadata: dict = None):
        """
        ...
        if metadata is None:
            metadata = {}
        self.logger = logging.getLogger(f"{__name__}_rank={str(comm.rank)}")
        ...
        if self.comm.rank == 0:
            ...
            write_metadata = {}
            import subprocess
            write_metadata["git_revision"] = subprocess.check_output(['git', 'rev-parse', 'HEAD'],
        ...
        self.gpu_ctx = KernelContext(device=comm.rank)
    ...
    def getNormalizedWeights(self):
        ...
        local_innovations = self._local_get_innovations()

        # Compute the gaussian pdf from the innovations
        local_gaussian_log_weights = self._local_get_gaussian_log_weights(local_innovations)
        ...
        if self.comm.rank == 0:
            ...
            global_normalized_weights = np.exp(global_gaussian_log_weights - max_log_weight) / np.exp(
                global_gaussian_log_weights - max_log_weight).sum()
    ...
    def resampleParticles(self, global_normalized_weights, write_to_file=False):
        ...
        resampling_indices = self._global_get_resampling_indices(global_normalized_weights)

        # Compute which particles should be overwritten by copies of other particles
        resampling_pairs = self._global_get_resampling_pairs(resampling_indices)
        ...
        for i in range(num_resample):
            ...
            if self.comm.rank == src_node and src_node == dst_node:
                self.logger.info(f"Node {src_node} copying internally {src} to {dst}")
                ...
                self.logger.info(f"Node {src_node} sending {src} to node {dst_node}")
                eta0, hu0, hv0 = self.ensemble.particles[local_src].download()
                eta1, hu1, hv1 = self.ensemble.particles[local_src].downloadPrevTimestep()
                ...
                self.logger.info(f"Node {dst_node} receiving {src} from node {src_node}")
                # FIXME: hard coded ghost cells here
                data = [local_dst]
                for j in range(6):
                    buffer = np.empty((self.ensemble.data_args['ny'] + 4, self.ensemble.data_args['nx'] + 4),
                                      dtype=np.float32)
        ...
        for local_dst, eta0, hu0, hv0, eta1, hu1, hv1 in receive_data:
            ...
            self._perturb_particle(local_dst, stream=stream)
    ...
    def _local_get_innovations(self):
    ...
    def _local_get_gaussian_log_weights(self, local_innovations):
        """
    ...
    def _global_get_resampling_indices(self, global_gaussian_weights):
        """
        ...
        if self.comm.rank == 0:
            ...
            weights_times_n = global_gaussian_weights * num_particles
            weights_times_n_integer = np.int64(np.floor(weights_times_n))
            deterministic = np.repeat(all_indices, weights_times_n_integer)

            # Stochastic resampling based on the decimal parts of N*weights:
            decimal_weights = np.mod(weights_times_n, 1)
            decimal_weights = decimal_weights / np.sum(decimal_weights)

            stochastic = np.random.choice(all_indices, num_particles - len(deterministic), p=decimal_weights)

            resampling_indices = np.sort(np.concatenate((deterministic, stochastic)))

        return resampling_indices

    def _global_get_resampling_pairs(self, global_resampling_indices):
        if self.comm.rank == 0:
            num_particles = self.num_nodes * self.local_ensemble_size
            ...
            self.logger.debug(f"Overwriting {str(dst)} on {str(dst_node)} with {str(src)} from {str(src_node)}")
    ...
    def dumpParticleInfosToFiles(self, prefix="particle_info"):
        ...
        """
        assert (self.ensemble.particleInfos[
                    0] is not None), 'particleInfos[0] is None, and dumpParticleInfosToFile was called... This should not happend.'
    ...
    def dumpDrifterForecastToFiles(self, prefix="forecast_observation_files"):
        ...
        """
        assert (self.ensemble.drifterForecast[
                    0] is not None), ' drifterForecast[0] is None, and dumpDrifterForecastToFiles was called... This should not happend.'
    ...
    def initDriftersFromObservations(self):
        self.ensemble.attachDrifters(
            self.observations.get_drifter_position(self.t, applyDrifterSet=False, ignoreBuoys=True))

    def _init_perturbators(self):
        ...
        for interpolation_factor in [3, 5, 7, 9]:
            try:
                self.perturbators[successful_state_noise_objects] = OceanStateNoise.OceanStateNoise.fromsim(
                    self.ensemble.particles[0],
                    soar_q0=2.5e-5,
                    interpolation_factor=interpolation_factor,
                    use_lcg=True)
                print('perturbator with interpolation factor ' + str(interpolation_factor) + ' is ' + str(
                    self.perturbators[successful_state_noise_objects].soar_q0))
    ...
    def _perturb_particle(self, particle_id, stream=None):
        perturbator_id = np.random.randint(0, 4)
        q0_scale = 10 * np.random.rand()
        self.perturbators[perturbator_id].perturbSim(self.ensemble.particles[particle_id], q0_scale=q0_scale,
                                                     stream=stream)

    def perturbAllParticles(self):
        stream = self.ensemble.particles[0].gpu_stream
        for p in range(len(self.ensemble.particles)):
            self._perturb_particle(p, stream=stream)
