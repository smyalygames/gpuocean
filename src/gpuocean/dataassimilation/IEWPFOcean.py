...
from gpuocean.utils import config, EnsemblePlot
from gpuocean.SWEsimulators import OceanStateNoise
from gpuocean.utils.gpu import GPUHandler, GPUStream, Array2D, Event
...
class IEWPFOcean:
    ...
    def __init__(self, ensemble: OceanNoiseEnsemble.OceanNoiseEnsemble, debug=False, show_errors=False,
        ...
        self.master_stream = GPUStream()

        self.debug = debug
        self.show_errors = show_errors

        # Store information needed internally in the class
        self.dx = ensemble.getDx()
        self.dy = ensemble.getDy()
        self.nx = ensemble.getNx()
        self.ny = ensemble.getNy()

        # Check that we have a supported model error term
        assert (ensemble.particles[
                    0].model_error is not None), "The ensemble members seem not to have a model error object"
        model_error_name = ensemble.particles[0].model_error.__class__.__name__
        assert (
                    model_error_name == "OceanStateNoise"), "Found model error of type " + model_error_name + ", but only supported model error type is OceanStateNoise"

        self.interpolation_factor = int(ensemble.particles[0].model_error.interpolation_factor)

        # Check that the interpolation factor plays well with the grid size:
        assert (
                    self.interpolation_factor > 0 and self.interpolation_factor % 2 == 1), 'interpolation_factor must be a positive odd integer'
        ...
        self.coarse_nx = int(self.nx / self.interpolation_factor)
        self.coarse_ny = int(self.ny / self.interpolation_factor)
        self.coarse_dx = float(self.dx * self.interpolation_factor)
        self.coarse_dy = float(self.dy * self.interpolation_factor)

        self.soar_q0 = float(ensemble.particles[0].model_error.soar_q0)
        self.soar_L = float(ensemble.particles[0].model_error.soar_L)
        self.f = float(ensemble.particles[0].f)
        self.g = float(ensemble.particles[0].g)
        ...
        self.const_H = float(H[0, 0])

        self.boundaryConditions = ensemble.particles[0].boundary_conditions

        self.geoBalanceConst = self.g * self.const_H / (2.0 * self.f)

        self.Nx = self.nx * self.ny * 3  # state dimension
        self.random_numbers_ratio = self.Nx / (self.coarse_nx * self.coarse_ny)

        self.numParticles = ensemble.getNumParticles()
        self.numDrifters = ensemble.getNumDrifters()
        ...
        assert (
            self.boundaryConditions.isPeriodicNorthSouth()), 'IEWPF requires periodic boundary conditions in north-south'
        assert (
            self.boundaryConditions.isPeriodicEastWest()), 'IEWPF requires periodic boundary conditions in east-west'
        # 3) that the Coriolis force is constant for the entire domain:
        assert (ensemble.particles[
                    0].coriolis_beta == 0), 'IEWPF requires constant Coriolis forcing, but got beta = ' + str(
            ensemble.beta)
        # 4) that dx and dy are the same
        assert (self.dx == self.dy), 'IEWPF requires square grid cells, but got (dx, dy) = ' + str((self.dx, self.dy))

        # Note that we intentionally do not add the ensemble as a member variable.
        ...
        self.S_device = Array2D(self.master_stream, 2, 2, 0, 0, self.S_host)
        ...
        self.localSVD_device = Array2D(self.master_stream, 49, 49, 0, 0, self.localSVD_host)

        self.iewpf_kernels = self.gpu_ctx.get_kernel("iewpf_kernels",
                                                     defines={'block_width': block_width, 'block_height': block_height})

        # Get CUDA functions and define data types for prepared_{async_}call()
        self.setBufferToZeroKernel = GPUHandler(self.iewpf_kernels, "setBufferToZero", "iiPi")
        self.blas_xaxpbyKernel = GPUHandler(self.iewpf_kernels, "blas_xaxpby", "iiPiPiff")
        self.halfTheKalmanGainKernel = GPUHandler(self.iewpf_kernels, "halfTheKalmanGain", "iiffffiifffPi")
        self.localSVDOnGlobalXiKernel = GPUHandler(self.iewpf_kernels, "localSVDOnGlobalXi", "iiiiPiPi")
    ...
    def iewpf_2stage(self, ensemble, infoPlots=None, it=None, perform_step=True):
        ...
        # TODO add a universal memory info for both HIP and CUDA
        ...
        self.log('(active particles, total number of particles): ' + str(
            (ensemble.getNumActiveParticles(), ensemble.getNumParticles())))
            ...
            if ensemble.particlesActive[p]:
                ...
                if np.isnan(phi_array[p]) or np.isnan(nu_norm_array[p]) or np.isnan(gamma_array[p]):
                    ensemble.deactivateParticle(p, msg='Failed with the Kalman gain, ' + \
                                                       '(phi, gamma, nu_norm): ' + str(
                        (phi_array[p], gamma_array[p], nu_norm_array[p])))
    ...
    def iewpf_timer(self, ensemble, infoPlots=None, it=None, perform_step=True):
        ...
        deterministic_step_event = Event()
    ...
    def setNoiseBufferToZero(self, sim):
        ...
        """
        self.setBufferToZeroKernel.async_call(self.noise_buffer_domain,
    ...
    def addBetaNuIntoAlphaXi(self, sim, alpha, beta):
        ...
        self.blas_xaxpbyKernel.async_call(self.noise_buffer_domain,
                                          self.local_size_domain,
                                          sim.gpu_stream,
                                          [sim.model_error.rand_nx,
                                           sim.model_error.rand_ny,
    ...
    def addKalmanGain(self, sim, all_observed_drifter_positions, innovation, drifter_id=None):
        ...
        for drifter in range(self.numDrifters):
            ...
            cell_id_x = int(np.floor(observed_drifter_position[0] / self.dx))
            cell_id_y = int(np.floor(observed_drifter_position[1] / self.dy))
            ...
            self.halfTheKalmanGainKernel.async_call(self.global_size_Kalman,
    ...
    def applyLocalSVDOnGlobal(self, sim,
        ...
        self.localSVDOnGlobalXiKernel.async_call(self.global_size_SVD,
                                                 self.local_size_SVD,
                                                 sim.gpu_stream,
                                                 [self.coarse_nx, self.coarse_ny,
    ...
    def solveImplicitEquation(self, gamma,
        ...
        try:
            ...
            self.log("Discrepancy with alpha_zero: " + str(
                self._implicitEquation_no_limit(alpha_zero, gamma, self.Nx, c_star)))
    ...
    def _createS(self, ensemble):
        ...
        # The values outside the 7x7 buffers are all zero, so we don't have to loop over them.
    ...
    def drawFromP_CPU(self, sim, observed_drifter_pos):
        ...
        if self.debug:
            ...
            self.showMatrices(p_eta[1:-1, 1:-1] - q_eta[1:-1, 1:-1], p_hu - q_hu,
                              "diff sample from P and sample from Q", p_hv - q_hv)
    ...
    def applyKalmanGain_CPU(self, sim,
        ...
        for drifter in range(sim.drifters.getNumDrifters()):
            ...
            north_east_south_west_index = [[4, 3, 0], [3, 4, 1], [2, 3, 2],
                                           [3, 2, 3]]  # [[y-index-eta, x-index-eta, index-local_eta_soar]]
            ...
            if self.debug: self.showMatrices(K_eta_tmp, local_eta,
                                             "global K_eta from local K_eta, halfway in the calc.")
            ...
            total_K_hu += K_hu
            total_K_hv += K_hv
            if self.debug: self.showMatrices(total_K_eta, total_K_hu, "Total Kalman gain after drifter " + str(drifter),
                                             total_K_hv)
    ...
    def applyScaledPSample_CPU(self, sim, eta_a, hu_a, hv_a,
        ...
        if self.debug: print("Check a against the Lambert W requirement: ", a, " < ",
                             - self.Nx + gamma - self.Nx * np.log(gamma / self.Nx), " = ",
                             a < - self.Nx + gamma - self.Nx * np.log(gamma / self.Nx))
        ...
        if self.debug: print("The two branches from Lambert W: ",
                             (lambertw(lambert_W_arg), lambertw(lambert_W_arg, k=-1)))
        if self.debug: print("The two branches from Lambert W: ",
                             (np.real(lambertw(lambert_W_arg)), np.real(lambertw(lambert_W_arg, k=-1))))
        ...
        if lambert_W_arg > (-1.0 / np.exp(1)):
            ...
            print("The requirement is lamber_W_arg > (-1.0/exp(1)): " + str(lambert_W_arg) + " > " + str(
                -1.0 / np.exp(1.0)))
            print("gamma: ", gamma)
            print("Nx: ", self.Nx)
            print("w_rest: ", w_rest)
        ...
        # self.debug = True
        if self.debug: print("--------------------------------------")
        if self.debug: print("Obtained (lambert_ans k=0, lambert_ans k=-1): ",
                             (lambertw(lambert_W_arg), lambertw(lambert_W_arg, k=-1)))
