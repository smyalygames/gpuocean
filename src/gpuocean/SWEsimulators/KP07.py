...
class KP07(Simulator.Simulator):
    ...
    def __init__(self,
                 ...
                 eta0: float, H: np.ma.MaskedArray, hu0: npt.NDArray[np.float32], hv0: npt.NDArray[np.float32],
                 nx: int, ny: int,
        ...
        if boundary_conditions.isSponge():
            nx = nx + int(boundary_conditions.spongeCells.east) + int(
                boundary_conditions.spongeCells.west) - 2 * ghost_cells_x
            ny = ny + int(boundary_conditions.spongeCells.north) + int(
                boundary_conditions.spongeCells.south) - 2 * ghost_cells_y
        ...
        # Get kernels
        self.kp07_kernel = gpu_ctx.get_kernel("KP07_kernel",
                                              defines=defines,
        ...
        self.swe_2D = GPUHandler(self.kp07_kernel, "swe_2D", "iifffffffffiPiPiPiPiPiPiPiPiiiiiPPPPf")
        self.update_wind_stress(self.kp07_kernel)

        # Upload Bathymetry
        self.bathymetry = Bathymetry(self.gpu_ctx, self.gpu_stream,
    ...
    def fromfilename(cls, gpu_ctx: KernelContext, filename: str, cont_write_netcdf=True):
        ...
        minmod_theta = sim_reader.get("minmod_theta")
        time_integrator = sim_reader.get("time_integrator")
        ...
        boundary_conditions = sim_reader.getBC()
        ...
        return cls(gpu_ctx,
                   ...
                   g, f, r,
                   t=time0,
                   theta=minmod_theta, use_rk2=using_rk2,
                   coriolis_beta=beta,
    ...
    def step(self, t_end=0.0):
        ...
        for i in range(0, n):
            ...
            wind_stress_t = float(self.update_wind_stress(self.kp07_kernel))

            if local_dt <= 0.0:
                break

            if self.use_rk2:
                self.swe_2D.async_call(self.global_size, self.local_size, self.gpu_stream,
                                       [self.nx, self.ny,
                                        ...
                                        self.gpu_data.h1.pointer, self.gpu_data.h1.pitch,
                                        self.gpu_data.hu1.pointer, self.gpu_data.hu1.pitch,
                                        self.gpu_data.hv1.pointer, self.gpu_data.hv1.pitch,
                                        self.bathymetry.Bi.pointer, self.bathymetry.Bi.pitch,
                                        self.bathymetry.Bm.pointer, self.bathymetry.Bm.pitch,
                                        self.boundary_conditions.north.value, self.boundary_conditions.east.value,
                                        self.boundary_conditions.south.value, self.boundary_conditions.west.value,
