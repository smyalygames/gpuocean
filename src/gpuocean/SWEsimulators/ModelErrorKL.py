...
class ModelErrorKL(object):
    ...
    def __init__(self, gpu_ctx: KernelContext, gpu_stream: GPUStream,
        ...
        self.klSamplingKernelEta = GPUHandler(self.kernels, "kl_sample_eta", "iiiiiiiiffffffPiPi")
        self.klSamplingKernel = GPUHandler(self.kernels, "kl_sample_ocean_state", "iifffffPPiiiiiiffffffPiPiPiPiPif")
    ...
    def setRandomNumbers(self, random_numbers):
        assert (random_numbers.shape == self.random_numbers_host.shape), "Wrong shape of random numbers: " + str(
            random_numbers.shape) + " vs " + str(self.random_numbers_host.shape)
    ...
    def _setRollers(self, roll_x_sin, roll_y_sin, roll_x_cos, roll_y_cos):
        ...
        def _check_roller(roller, name):
            assert (isinstance(roller, (float, int,
                                        np.float32)) and 0 <= roller <= 1), "illegal type/value of " + name + ": " + str(
                roller)
    ...
    def perturbEta(self, eta,
        ...
        if random_numbers is not None:
            assert (isinstance(random_numbers, (Array2D,
                                                np.ndarray))), "random numbers should be a CUDA 2D array or a numpy array, but is " + str(
                type(random_numbers))
            if isinstance(random_numbers, Array2D):
        ...
        self.klSamplingKernelEta.async_call(self.global_size_KL_eta, self.local_size, stream,
                                            [self.nx, self.ny,
                                             self.basis_x_start, self.basis_x_end,
                                             self.basis_y_start, self.basis_y_end,
                                             self.include_cos, self.include_sin,
                                             self.kl_decay,
                                             perturbation_scale * self.kl_scaling,
                                             self.roll_x_sin, self.roll_y_sin,
                                             self.roll_x_cos, self.roll_y_cos,
    ...
    def perturbOceanState(self, eta, hu, hv, H,
        ...
        self.klSamplingKernel.call(self.global_size_KL, self.local_size,
                                   [self.nx, self.ny, dx, dy,
                                    ...
                                    self.include_cos, self.include_sin,
                                    self.kl_decay,
                                    float(perturbation_scale * self.kl_scaling),
                                    self.roll_x_sin, self.roll_y_sin,
                                    self.roll_x_cos, self.roll_y_cos,
    ...
    def perturbSimSimilarAs(self, simToPerturb, simSource=None, modelError=None, stream=None,
        ...
        assert (
                    simSource is not None or modelError is not None), "Please provide either simSource or modelError input arguments"
        assert (
                    simSource is None or modelError is None), "Please provide only one of simSource or modelError, not both."
        ...
        if self.KL_basis_fields_sin is None and self.include_sin:
            ...
            for n in range(0, self.N_basis_y):
                for m in range(0, self.N_basis_x):
                    ...
                    self.KL_basis_fields_sin[n, m, :, :] = (
                                self.kl_scaling * basis_m ** (-self.kl_decay) * basis_n ** (-self.kl_decay) *
                                np.outer(np.sin(
                                    2 * basis_m * np.pi * np.linspace(-half_unit_dy, 1 + half_unit_dy, self.ny + 2)),
                                         np.sin(2 * basis_n * np.pi * np.linspace(-half_unit_dx, 1 + half_unit_dx,
                                                                                  self.nx + 2))))

            self.KL_basis_fields_sin = np.reshape(self.KL_basis_fields_sin,
                                                  (self.N_basis_y * self.N_basis_x, self.ny + 2, self.nx + 2))
        ...
        if self.KL_basis_fields_cos is None and self.include_cos:
            ...
            for n in range(0, self.N_basis_y):
                for m in range(0, self.N_basis_x):
                    basis_n = self.basis_y_start + n
                    basis_m = self.basis_x_start + m
                    self.KL_basis_fields_cos[n, m, :, :] = (
                                self.kl_scaling * basis_m ** (-self.kl_decay) * basis_n ** (-self.kl_decay) *
                                np.outer(np.cos(
                                    2 * basis_m * np.pi * np.linspace(-half_unit_dy, 1 + half_unit_dy, self.ny + 2)),
                                         np.cos(2 * basis_n * np.pi * np.linspace(-half_unit_dx, 1 + half_unit_dx,
                                                                                  self.nx + 2))))
            self.KL_basis_fields_cos = np.reshape(self.KL_basis_fields_cos,
                                                  (self.N_basis_y * self.N_basis_x, self.ny + 2, self.nx + 2))
    ...
    def perturbEtaCPU(self, eta, use_existing_GPU_random_numbers=False, random_numbers=None,
        ...
        assert (eta.shape == (self.ny + 4, self.nx + 4)), "expected eta to be shape " + str(
            (self.ny + 4, self.nx + 4)) + " but got " + str(eta.shape)
    ...
    def perturbOceanStateCPU(self, eta, hu, hv, Hi, f, beta=0.0, g=9.81,
        ...
        assert (eta.shape == (self.ny + 4, self.nx + 4)), "expected eta to be shape " + str(
            (self.ny + 4, self.nx + 4)) + " but got " + str(eta.shape)
        assert (hu.shape == (self.ny + 4, self.nx + 4)), "expected hu to be shape " + str(
            (self.ny + 4, self.nx + 4)) + " but got " + str(hu.shape)
        assert (hv.shape == (self.ny + 4, self.nx + 4)), "expected hv to be shape " + str(
            (self.ny + 4, self.nx + 4)) + " but got " + str(hv.shape)
        ...
        d_eta = self._CPUSampleKL(use_existing_GPU_random_numbers=use_existing_GPU_random_numbers,
                                  ...
                                  roll_x_sin=roll_x_sin, roll_y_sin=roll_y_sin,
                                  roll_x_cos=roll_x_cos, roll_y_cos=roll_y_cos)
        assert (d_eta.shape == (self.ny + 2, self.nx + 2)), "expected d_eta to be shape " + str(self.ny + 2,
                                                                                                self.nx + 2) + " but got " + str(
            d_eta.shape)
    ...
    def _CPUSampleKL(self, use_existing_GPU_random_numbers=False, random_numbers=None,
        ...
        # We transpose the random numbers so that they correspond with the
        # GPU kernel
    ...
    def _applyBCSingleGhost(self, data):
        assert (data.shape == (self.ny + 2, self.nx + 2)), "Expected data to be of shape " + str(
            (self.ny + 2, self.nx + 2)) + " while got " + str(data.shape)
