...
class CombinedCDKLM16:
    ...
    def __init__(self, barotropic_sim: CDKLM16.CDKLM16, baroclinic_sim: CDKLM16.CDKLM16):
        ...
        for cardinal in ["north", "south", "east", "west"]:
            assert getattr(barotropic_sim.boundary_conditions, cardinal) == getattr(baroclinic_sim.boundary_conditions,
                                                                                    cardinal), "sims do NOT match: bc"
    ...
    def fromdataargs(cls,
                     barotropic_gpu_ctx: KernelContext, baroclinic_gpu_ctx: KernelContext,
                     barotropic_eta0: npt.NDArray[np.float32],
                     barotropic_hu0: npt.NDArray[np.float32], barotropic_hv0: npt.NDArray[np.float32],
                     barotropic_H: npt.NDArray[np.float32],
                     baroclinic_eta0: npt.NDArray[np.float32],
                     baroclinic_hu0: npt.NDArray[np.float32], baroclinic_hv0: npt.NDArray[np.float32],
                     baroclinic_H: npt.NDArray[np.float32],
                     nx: int, ny: int,
                     dx: float, dy: float, dt: float,
                     barotropic_g: float, baroclinic_g: float, f: float, r: float,
        ...
        if sim_flags["barotropic"]:
            barotropic_sim = CDKLM16.CDKLM16(barotropic_gpu_ctx,
                                             ...
                                             netcdf_filename="barotropic_" + datetime.datetime.now().strftime(
                                                 "%Y_%m_%d-%H_%M_%S"),
                                             ...
                                             num_threads_dt=num_threads_dt,
                                             block_width_model_error=block_width_model_error,
                                             block_height_model_error=block_height_model_error)
        ...
        if sim_flags["baroclinic"]:
            baroclinic_sim = CDKLM16.CDKLM16(baroclinic_gpu_ctx,
                                             ...
                                             local_particle_id=local_particle_id,
                                             super_dir_name=super_dir_name,
                                             netcdf_filename="baroclinic" + datetime.datetime.now().strftime(
                                                 "%Y_%m_%d-%H_%M_%S"),
                                             ignore_ghostcells=ignore_ghostcells,
                                             courant_number=courant_number,
                                             offset_x=offset_x, offset_y=offset_y,
    ...
    def combinedStep(self, t_end=0.0, apply_stochastic_term=True, write_now=True, update_dt=False, trajectory_dt=0.0,
                     trajectories=None,
        ...
        while (baroclinic_t_now < t_end) or (barotropic_t_now < t_end):
            ...
            if baroclinic_t_now <= barotropic_t_now:
                ...
                baroclinic_local_dt = float(min(self.baroclinic_sim.dt, float(t_end - baroclinic_t_now)))
                barotropic_local_dt = 0.0
            else:
                baroclinic_local_dt = 0.0
                if update_dt:
                    self.barotropic_sim.updateDt()
                barotropic_local_dt = float(min(self.barotropic_sim.dt, float(t_end - barotropic_t_now)))

            # Simulate step
            if baroclinic_local_dt > 0.0:
                self.baroclinic_sim.step(baroclinic_local_dt, apply_stochastic_term=apply_stochastic_term,
                                         write_now=False)
                baroclinic_t_now += np.float64(baroclinic_local_dt)
                self.baroclinic_iters += 1
            elif barotropic_local_dt > 0.0:
                self.barotropic_sim.step(barotropic_local_dt, apply_stochastic_term=apply_stochastic_term,
                                         write_now=False)
            ...
            if barotropic_local_dt > 0.0:
                ...
                if (trajectories is not None) and (baroclinic_t_now > trajectory_t) and (
                        barotropic_t_now > trajectory_t):
